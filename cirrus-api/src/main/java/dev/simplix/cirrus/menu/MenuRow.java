...
public class MenuRow implements Iterable<MenuElement> {
  ...
  public MenuRow(List<MenuElement> items) {
    if (items.size() != 9) {
      throw new IllegalArgumentException("Items#size must be 9 but is "
                                         + items.size()
                                         + ". There are 9 items in a menu-row");
    }
    this.items = items;
  }
   ...
   *
   * @param indexOfItemInRow The index of the item in the row (not the index of the item in the
   *                         whole menu)
